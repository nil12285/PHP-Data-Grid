...
class Jquerypager {
    ...
    {        
        ...
        $this->tableStart = "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\" id=\"".$this->idPager."\" class=\"table dataTable table-striped table-bordered $highLightClass\">\n";
        $this->tableEnd = "</table>";
    }
                ...
                foreach($aoColumns as $val){
                    ...
                    if(is_array($val)){
                        ...
                        foreach($val as $key1=>$val1){
                            ...
                            if($val1 === true && !is_null($key1))
                                $aoColumnsStr .= "\"".$key1."\" : " . 'true';
                            elseif($val1 === false && !is_null($key1))
                                $aoColumnsStr .= "\"".$key1."\" : " . 'false';
                            elseif($key1 == 'fnRender')
    		...
    		foreach($this->header as $hkey=>$hval){    		  
                if(isset($hval['object'])) {
                    if(is_object($hval['object'])) {
                        ...
                        $aRow[$hkey] = $aRow['object'];
                    }                    
                }
                
                array_push($aData,htmlentities($aRow[$hkey],ENT_QUOTES,'UTF-8'));                
    		}
                    ...
                    foreach($tableBodyData as $data){
                        ...
                        foreach($this->header as $key=>$value){
                            if(isset($value['object'])) {
                                if(is_object($value['object'])) {
                                    $data[$key] = $value['object']->$value['function']($data,$value['param']);
                                } else {
                        ...
                        if(!empty($this->tbLink)){
                            foreach($this->tbLink as $val2){
                                ...
                                $url = $this->_generateLink($link,$data,$val2);                                 
    ...
    private function _generateLink($link,$data,$linkData)
    {
        ...
        if(is_array($link['qString'])){
            foreach($link['qString'] as $qstr){
                if(!empty($qstr) && isset($data[$qstr]) && !empty($data[$qstr])){ 
                    if (preg_match('/^http:/i', $data[$qstr]))
                        $url = $data[$qstr];
                    else
                        $url .= "/".$data[$qstr];
                }
    ...
    private function _getLinkHtml($linkData,$url,$data=array())
    {
        ...
        elseif($linkData['target'] == "confirm"){
            $this->cnfFlag = true;
            $noAjaxAttr = '';
            if(isset($linkData['noAjax']) && $linkData['noAjax'] == true) {
                $noAjaxAttr = ' noAjax = "true" ';
            }
            //if(!empty($linkData['refresh-url']) && !empty($linkData['target-div'])){                
            if(!empty($linkData['refresh-url'])){
                $this->pagerRefreshFlag = true;
                $linkHtml .= "<a url=\"".$url."\" $noAjaxAttr class=\"ui-custom-link\" href=\"javascript: void(0);\" onclick=\"Javascript: openConfirm('".$uniqId."','".$linkData['refresh-url']."','".$linkData['target-div']."');\" id=\"".$uniqId."\" >";
            }
            else{
                $linkHtml .= "<a url=\"".$url."\" $noAjaxAttr class=\"ui-custom-link\" href=\"javascript: void(0);\" onclick=\"Javascript: openConfirm('".$uniqId."','','');\" id=\"".$uniqId."\" >";
            }
        }
